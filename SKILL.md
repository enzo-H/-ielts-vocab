...
- **页内再学循环（2026-08-31 v2）**：again 的词当轮自动进再学队列，过完一轮后页面
  自动发起下一轮（卡片带"再学 N"标签），直到全部过掉或用户点"先到这里，保存成绩"。
  一次保存包含全部轮次的 id=grade 串
- **保存后自查**：/record 响应含 remaining（最新 plan）。有剩余 → 保存按钮变
  "继续学习剩余 N 词"（点击 reload 拉新任务）；无剩余 → 显示"今日任务全部完成 ✓"。
  SM-2 层 today 重排的词也会出现在 remaining.due_reviews 里，闭环成立
- 逐词展示：word + 音标 + 词性 + 释义 + 双语例句 + **同义替换**（雅思核心考点）
- 回忆模式：先只显示单词，点"显示释义"后展开
